...
WORKDIR /frontend
COPY go-tangra-backup/frontend/package.json go-tangra-backup/frontend/pnpm-lock.yaml* ./
RUN pnpm install --frozen-lockfile || pnpm install
COPY go-tangra-backup/frontend/ .
RUN pnpm build
...
COPY go-tangra-backup/go.mod go-tangra-backup/go.sum ./
...
COPY --from=builder /src/bin/backup-server /app/bin/backup-server
...
RUN mkdir -p /data/backups/modules /data/backups/full
...
# Set default command
CMD ["/app/bin/backup-server", "-c", "/app/configs"]

# Labels
LABEL org.opencontainers.image.title="Backup Orchestrator Service" \
      org.opencontainers.image.description="Centralized backup and restore orchestration for all platform modules" \
